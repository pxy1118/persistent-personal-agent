...
import { locations, root, readConfig, atomicJson, cli, sessionArgs, memoryDir, json, connect, type Locations } from '../src/letta-runtime.js';
import { migrate, backupLetta, restoreLetta, filesUnder } from '../src/letta-data.js';
...
try {
  ...
  const c = readConfig(p), m = await migrate(p, c); const id = m.agentId!; report.agentId = id; save();
  ...
  report.status = 'LIVE_CORE_PASSED_PTY_AND_CANCEL_PENDING'; save();
