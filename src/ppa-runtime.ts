...
export const version = '0.31.12-ppa.1';
export const upstreamVersion = '0.31.12';
...
export function readConfig(p: Locations): LettaConfig {
  ...
  if (retired.length) console.error(`旧配置不再生效：${retired.join(', ')}。使用 PPA 运行时设置。`);
...
export function cliPath() {
  const entry = createRequire(import.meta.url).resolve('@ppa/runtime');
  const dir = dirname(entry);
  if (json(join(dir, 'package.json')).version !== version) throw new Error(`必须使用 PPA Runtime ${version}，请运行 npm ci。`);
  return entry;
}
...
export function cli(p: Locations, args: string[], timeout = 60000) {
  ...
  if (r.status !== 0) throw new Error(redact(`PPA Runtime ${args[0]} 失败：${r.error?.message ?? ''} ${r.stderr} ${r.stdout}`).slice(0, 2000));
