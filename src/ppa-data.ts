...
import { root, version, upstreamVersion, json, atomicJson, digest, cli, memoryDir, agentFile, configureAgent, connect, modelIds, modelHandle, type Locations, type LettaConfig } from './ppa-runtime.js';
...
export type Migration = { version: 1; status: 'preparing' | 'complete'; sourceFingerprint: string; oldAgentId: string; agentId?: string; sourceMemoryIds: string[]; createdAt: string; completedAt?: string; legacyBackup?: string; lettaVersion: string; ppaRuntimeVersion?: string; upstreamLettaVersion?: string };
...
export async function migrate(p: Locations, c: LettaConfig) {
  ...
  if (!record) {
    ...
    record = { version: 1, status: 'preparing', sourceFingerprint: fingerprint, oldAgentId: source.agentId, sourceMemoryIds: source.memories.map(m => m.id), createdAt: new Date().toISOString(), legacyBackup, lettaVersion: upstreamVersion, ppaRuntimeVersion: version, upstreamLettaVersion: upstreamVersion };
  ...
  if (!JSON.stringify(checked).includes(id)) throw new Error('PPA Runtime 无法加载迁移后的 Agent。');
...
export function backupLetta(p: Locations) {
  ...
  const target = join(p.backups, `ppa-runtime-${Date.now()}-${randomUUID().slice(0, 8)}`); mkdirSync(target, { recursive: true });
  ...
  return seal(target, { kind: 'letta', agentId: m.agentId, lettaVersion: upstreamVersion, ppaRuntimeVersion: version, upstreamLettaVersion: upstreamVersion, sourceData: p.data });
}
export function restoreLetta(source: string, target: string) {
  ...
  if (m.version !== 2 || m.kind !== 'letta' || !m.files || !m.files['letta-migration.json']) throw new Error('无效 PPA Runtime 备份。');
