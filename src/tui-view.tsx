...
export type TuiState = {
  ...
  online: boolean; modelReady: boolean; approval?: { tool: string; args: string; id: string };
  menu?: Menu; editor?: { path: string; text: string }; closing: boolean;
};
export type TuiActions = { submit: (text: string) => void; stop: () => void; quit: () => void; dismiss: () => void; save: (text: string) => void };
export const commands = [
  ['/help','命令与快捷键'],['/new','开始新对话'],['/sessions','切换会话'],['/model','切换模型配置'],['/persona','查看人格'],['/persona edit','编辑人格'],['/memory','查看与编辑记忆'],['/history','查看对话历史'],['/status','连接与工作区'],['/reconnect','重新连接模型'],['/quit','退出 PPA'],
];
...
function Composer({value,setValue,focus,onSubmit,onNavigate,onTab,multiline=false,onSave,suggesting=false}:{
  value:string;setValue:(v:string)=>void;focus:boolean;onSubmit:()=>void;onNavigate:(n:number)=>void;onTab:()=>void;multiline?:boolean;onSave?:()=>void;suggesting?:boolean;
}) {
...
export function TuiView({initial,subscribe,actions}:{initial:TuiState;subscribe:(f:(s:TuiState)=>void)=>()=>void;actions:TuiActions}) {
  const [state,setState]=useState(initial), [draft,setDraft]=useState(''), [selected,setSelected]=useState(0), [allow,setAllow]=useState(false);
  ...
  useInput((input,key)=>{
    if(key.ctrl && input==='c'){state.busy?actions.stop():actions.quit();return;}
  ...
  return <Box flexDirection="column" width={width}>
    <Static items={[{id:-1,role:'info' as const,text:''},...state.entries]}>
      ...
      </Box>:<Box key={entry.id} flexDirection="column" paddingX={2} marginBottom={1}>
    ...
    {state.approval?<Box flexDirection="column" borderStyle="round" borderColor="yellow" paddingX={2} marginTop={1}>
    ...
    </Box>:state.editor?<Box flexDirection="column" borderStyle="round" borderColor={accent} paddingX={1} marginTop={1}>
      <Text bold color={accent}>编辑 · {state.editor.path}</Text>
      <Box marginY={1}><Composer value={draft} setValue={setDraft} focus={!state.closing&&!state.busy} multiline onSubmit={()=>{}} onNavigate={()=>{}} onTab={()=>{}} onSave={()=>actions.save(draft)}/></Box>
      ...
      {items.length>0&&<Box flexDirection="column" paddingX={2} marginTop={1}>
        ...
        <Text color={muted}>↑ ↓ 选择 · {state.menu?'Enter 打开 · Esc 返回':'Tab 补全 · Enter 执行'}</Text>
      </Box>}
      {!state.menu&&<Box borderStyle="single" borderTop borderBottom borderLeft={false} borderRight={false} borderColor={state.busy?muted:accent} paddingX={2} marginTop={1} gap={1}>
        <Text color={accent}>›</Text><Box flexGrow={1}><Composer value={draft} setValue={setDraft} focus={!state.busy&&!state.closing} onSubmit={submit} suggesting={items.length>0} onNavigate={n=>setSelected(i=>(i+n+items.length)%items.length)} onTab={()=>{if(items[index])setDraft(items[index]!.command);}}/></Box>
      </Box>}
    </>}
    <Box justifyContent="space-between" paddingX={2} marginTop={state.menu?1:0}>
      <Text color={state.modelReady?accent:'yellow'}>● {state.closing?'正在退出':state.modelReady?'标准审批':'模型离线'}</Text>
      <Text color={muted} wrap="truncate-start">{shortModel(state.model)} · {state.name}</Text>
    </Box>
    <Box paddingX={2} marginBottom={1}><Text color={muted}>{state.editor?'人格与记忆由原生存储保存':state.approval?'请核对操作参数':state.busy?'回复中 · 输入不会自动排队':'Enter 发送 · Shift+Enter / Ctrl+N 换行 · /help 帮助'}</Text></Box>
  </Box>;
}
