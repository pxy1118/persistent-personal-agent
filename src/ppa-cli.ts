...
import { locations, readConfig, json, cli, modelIds, connect, configureAgent, launch, redact, isLocalModelEndpoint } from './ppa-runtime.js';
import { migrate, backupLetta, restoreLetta, type Migration } from './ppa-data.js';
    ...
    try {
      if (command === 'backup') console.log(backupLetta(p));
        ...
        if (action === 'use' || action === 'switch') {
          ...
          if (m?.status !== 'complete' || !m.agentId) throw new Error('请先运行 npm run migrate:ppa，初始化或迁移助手。');
        ...
        const c = readConfig(p);
        if (!existsSync(p.manifest)) throw new Error('请先运行 npm run migrate:ppa，初始化或迁移助手。');
        const m = json<Migration>(p.manifest);
        if (m.status !== 'complete' || !m.agentId) throw new Error('迁移未完成，请运行 npm run migrate:ppa。');
        await modelIds(c); connect(p, c); configureAgent(p, m.agentId, c);
        const result = await launch(p, m.agentId);
        process.exitCode = result.code;
      } else throw new Error(`未知命令：${command}。使用 PPA Runtime 维护入口管理记忆和人格。`);
    } finally { release(); }
  }
