...
- 聊天支持中文流式回复、图片、停止、历史会话、新对话、人格与记忆编辑、模型、权限模式和回复节奏。Enter 发送，Shift+Enter 换行；回复期间仍可编辑并发送，新消息会先中止旧回合再发送一次；Escape 只停止，不发送草稿。
...
`provider` 默认 `openai-compatible`，目前还支持 `llama-cpp`。PPA Runtime 会逐模型合并原生 `/models` 与 `/props?model=<id>` 的能力信息：已有上下文元数据但缺少模态声明时，仍补查视觉能力；明确的逐模型元数据保持优先，避免模型之间发生能力串用。PPA 现在直接连接配置的原始模型地址，不再启动本地模型代理。llama.cpp provider 会从 `/props` 识别支持思考的模板；只有模板明确支持时，运行时才发布 `thinking_budget_tokens` 和 Qwen 模板级开关，不把通用 `reasoning_effort` 当成能力证明。

### 自然回复节奏

`/rhythm native` 使用模型服务原生思考方式；`/rhythm adaptive` 启用按回合自适应节奏。首次和未完成真实后端验收时默认保持 `native`，可随时一键回退。桌宠完整聊天窗口提供相同的“原生节奏 / 自适应节奏”选择。

自适应模式先用同一 Agent、同一后端模型进行一次关闭显式 reasoning 的快速生成。简单接话可以直接结束；严谨判断可以先静默思考；倾诉伴随复杂问题时，助手可以先输出一句贴合语境的真实回应，再调用内部 `deliberate` 继续回答。这里没有额外分类请求、常驻小模型、人为延时、随机停顿或固定开场白。

普通回合采用分层工具面：模型始终能看到轻量的 `activate_capability` 能力目录，但不会为一句问候加载完整工具 Schema。目录明确列出文件读取/编辑、命令、读屏、持久记忆、任务和 Skill 都真实存在；需要操作时，模型选择最多三个能力组，Runtime 在同一个用户回合内强制建立阶段边界，再只挂载所选真实工具继续执行。这样“当前未挂载”不会被解释为“不存在”，原有工具审批规则也不变。能力阶段失败会显示运行时错误，不会伪装成助手没有该能力。

内部 `deliberate` 每个用户回合最多一次，不执行工具、不写记忆，也不显示推理正文。`standard` 最多使用 1,024 个 reasoning token，并为内部结论预留 512 token；`deep` 最多使用 4,096 个 reasoning token，并预留 768 token；两档最长均为 60 秒。模型选择是否使用及深度，运行时负责硬上限。续答会得到已经说过的内容，仅移除完全一致的重复前缀，不做模糊删句。

启用前会同时验证 llama.cpp 的 `/apply-template` 是否能逐请求切换 `enable_thinking`，并用极小预算请求检查可见正文和 reasoning 事件。服务离线、非 llama.cpp、模板不支持或预算探测失败时，自适应模式不会静默生效：启动时回退到原生模式并显示原因，手动切换则直接报错。`reasoning-preserve` 的服务端行为保持不变。
...
| `interaction.json` | `native / adaptive` 回复节奏选择，随数据目录保存 |
...
直接自然聊天，Enter 发送。回复过程中仍可编辑；此时按 Enter 会立即停止显示旧回合的新内容，请求后台取消，确认终止后把新输入发送一次。若取消未确认，草稿会保留且不会并行启动新回合。Escape 或 `/stop` 只中断，不发送草稿；旧回合迟到的事件会按回合标识丢弃。已经完成的工具副作用及真实回执不会撤销或重放。`/quit`、`exit` 或空闲时 Ctrl+C 正常退出；记忆编辑有未保存内容时 Ctrl+C 会先询问。
...
| `/rhythm` / `/rhythm native\|adaptive` | 查看并切换原生或自适应回复节奏 |
...
独立 reasoning 通道只显示思考状态，不打印内容；回应、思考、执行工具、等待审批和结束使用同一回合标识同步到终端与桌宠，正文一出现就退出“正在思考”状态。若某个后端把思考内容伪装成普通正文，PPA 不会用语言或模糊规则删句；这种服务不能据此认定为自适应能力可用。
...
npm run smoke:adaptive          # 隔离身份，native/adaptive 同提示各重复 3 次并记录阶段、首字延迟和 token
npm run smoke:adaptive -- --quick  # 每类 1 次，仅用于快速诊断，不构成正式验收
npm run smoke:capabilities      # 隔离身份，验证轻量上下文、能力激活和真实 Read 调用
```

自适应报告写入 `.ppa/reports/adaptive-thinking-live.json`，测试 Agent、人格、会话和工具文件位于单独的 `.ppa/adaptive-live-*` 数据目录，不进入正式助手。脚本覆盖问候、简短确认、倾诉加分析、纠正、复杂比较、只读工具和图片；正式运行每类每种模式重复三次，并保留原始回答供人工检查。报告状态 `LIVE_REVIEW_REQUIRED` 表示自动采集完成但仍需人工评价三种节奏、重复开场和回答质量；只有完成这一步后才应把正式数据目录切到 `adaptive`。

2026-09-09 使用当前 Qwen3.8 EfficientThink/llama.cpp 完成 42/42 个隔离回合。请求级 `enable_thinking` 与 `thinking_budget_tokens` 能力验证通过，未发现内部 reasoning 泄漏；原生模式有 5 个回合在 120 秒窗口内没有可见正文，自适应模式 21 个回合均有正文。简单交流的自适应首字中位数约 18.17 秒、P95 约 18.27 秒，明显好于原生的约 26.35 秒和 58.67 秒，但没有达到 2 秒/5 秒目标；无 reasoning 的简单回合仍需约 18 秒，主要瓶颈是约 23k 上下文的服务端预填充。

该后端可以稳定到达“直接回答”和“先静默思考再回答”，但不能可靠生成“普通正文后紧接工具调用”：正式对照中 4 次 `deliberate` 均没有提前正文，额外 3 次针对性测试也全部先发空正文工具调用。即使直接要求先输出指定句子，服务仍返回空 `content` 加纯 tool call。因此 `adaptive` 继续作为可手动启用的实验模式，正式默认保持 `native`；PPA 不以固定 UI 开场、工具参数中的开场或人为等待伪造第三种节奏。

分层工具面的真实隔离报告位于 `.ppa/reports/capability-routing-live.json`。2026-09-09 当前 Qwen3.8/llama.cpp 验收中，普通问候的 Runtime 上下文估算从旧全量工具面的约 23,581 降到 8,439 token；模型对读文件请求主动激活 `files_read`，第二阶段只挂载目标工具并真实调用 `Read`，最终返回文件中的验收词，没有声称能力不存在。该单次问候首字约 6.94 秒，读文件完整回合约 20.24 秒；这是功能和上下文缩减证据，不是长期延迟统计。

本次功能回归通过：根项目测试 45 项、PPA Runtime 聚焦回归 15 项、桌宠 Python 测试 32 项、PPA 接口集成 18 项，根项目与 Runtime 构建也均通过。PPA 接口集成覆盖轻量能力目录、按需挂载真实工具、三种回复节奏、流式聊天、会话恢复、图片消息、读屏工具结果、记忆提交和冲突保护、文件审批/拒绝、思考期间权限模式切换及重启保留、发送即打断且新消息只发送一次、模型切换和离线记忆访问。
