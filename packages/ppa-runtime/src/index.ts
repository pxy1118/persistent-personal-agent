...
function printHelp() {
  ...
  const usage = `
PPA Runtime is the stateful agent backend maintained for Persistent Personal Agent

USAGE
  # interactive TUI
  ppa-runtime           Resume last conversation for this project
  ppa-runtime --new           Create a new conversation (for concurrent sessions)
  ppa-runtime --resume        Open agent selector UI to pick agent/conversation
  ppa-runtime --new-agent     Create a new agent directly (skip profile selector)
  ppa-runtime --agent <id>    Open a specific agent by ID

  # headless
  ppa-runtime -p "..."        One-off prompt in headless mode (no TTY UI)

  # maintenance
  ppa-runtime update          Check for updates and install (aliases: upgrade, --update, --upgrade)
  ppa-runtime memory ...      Memory filesystem subcommands
  ppa-runtime agents ...      Agents subcommands (JSON-only)
  ppa-runtime computers ...   List available remote computers (JSON-only)
  ppa-runtime teleport ...    Move the current conversation between computers
  ppa-runtime messages ...    Messages subcommands (JSON-only)
  ppa-runtime mcp ...         List, search, and call MCP servers available to an agent
  ppa-runtime mods ...        List and manage local mods
  ppa-runtime sandbox ...     Transfer files to or from the current Cloud sandbox
  ppa-runtime server ...      Run a remote computer, channels, or the App Server
  ppa-runtime connect ...     Connect providers from terminal
  ppa-runtime backend ...     Show or set the default backend
  ppa-runtime setup           Re-run first-run setup
  ppa-runtime install ...     Install a skill or mod package
  ppa-runtime skills ...      List or delete installed agent skills
OPTIONS
${renderCliOptionsHelp()}
SUBCOMMANDS
  ppa-runtime memory status --agent <id>
  ppa-runtime memory diff --agent <id>
  ppa-runtime memory resolve --agent <id> --resolutions '<JSON>'
  ppa-runtime memory backup --agent <id>
  ppa-runtime memory backups --agent <id>
  ppa-runtime memory restore --agent <id> --from <backup> --force
  ppa-runtime memory export --agent <id> --out <dir>
  ppa-runtime memory pull --agent <id>
  ppa-runtime memory tokens [--memory-dir <path>] [--agent <id>] [--format text|json]
  ppa-runtime agents list [--query <text> | --name <name> | --tags <tags>]
  ppa-runtime computers list [--online-only]
  ppa-runtime computers current
  ppa-runtime teleport list|cloud|local|<computer>
  ppa-runtime messages search --query <text> [--all-agents]
  ppa-runtime messages list [--agent <id>]
  ppa-runtime messages transcript --conversation <id> [--out <path>]
  ppa-runtime mods list [--agent <id>]
  ppa-runtime mods package <mod-file> --name <package-name> [--out <dir>]
  ppa-runtime mods enable <package-spec>
  ppa-runtime mods disable <package-spec>
  ppa-runtime mods remove <package-spec>
  ppa-runtime mcp list|get|tools|search|call ... [--agent <id>]
  ppa-runtime server [--computer-name <name> | --listen [url]] [options]
  ppa-runtime connect <provider> [options]
  ppa-runtime install <thing> [--agent <id> | -n <name>]
  ppa-runtime skills list [--agent <id> | -n <name>]
  ppa-runtime skills delete <skill_name> --agent <id>
  ppa-runtime backend [cloud|local]
  ppa-runtime local-backend migrate-transcripts [--storage-dir <path>] [--dry-run]

BEHAVIOR
  On startup, PPA Runtime checks for saved profiles:
...
EXAMPLES
  ...
  ppa-runtime              # Show profile selector or create new
  ppa-runtime --new              # Create new conversation
  ppa-runtime --agent agent_123  # Open specific agent
  ppa-runtime install official/finance/stocks --agent agent-123
  ppa-runtime install npm:@letta-ai/mod-plan-mode
  ...
  ppa-runtime -p "hello" --output-format json
...
async function printInfo() {
  ...
  console.log(`PPA Runtime ${getVersion()}\n`);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error("Run 'ppa-runtime --help' for usage information.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Usage: ppa-runtime --agent agent-xyz --conv default");
    console.error("   or: ppa-runtime --conv agent-xyz (shorthand)");
    process.exit(1);
  }
  ...
  if (fromAfFile) {
    ...
    if (fromAfFile.startsWith("@")) {
      ...
      try {
        ...
        console.error(
          `Error: Invalid registry handle "${fromAfFile}". Use format: ppa-runtime --import @author/agentname`,
        );
        process.exit(1);
      }
    } else {
      ...
      if (!isValid) {
        ...
        if (isHeadless) {
          ...
          if (process.env.LETTA_API_KEY) {
            ...
            console.error("Run `ppa-runtime setup` to re-authenticate.");
          }
          process.exit(1);
        }
          ...
          case "create": {
            ...
            try {
              const defaultAgent = await ensureDefaultAgents(getBackend(), {
                ...
                // gets the standard PPA Runtime agent.
                personality:
        ...
        if (backend.capabilities.remoteMemfs && !autoEnableMemfsForFreshAgent) {
          ...
          if (!memfsEnabled) {
            if (await isLettaCloud()) {
              ...
              // Agents can be created outside PPA Runtime without the tag.
          ...
          void ensureLettaCodeOriginTag(agent)
            .catch((error) => {
              import("@/utils/debug").then(({ debugWarn }) =>
                debugWarn(
                  "startup",
                  `Failed to ensure PPA Runtime origin tag for ${agent.id}: ${
                    error instanceof Error ? error.message : String(error)
                  }`,
                ),
              );
