...
export function SetupUI({
  ...
  const [doneMessage, setDoneMessage] = useState("Starting PPA Runtime...");
  ...
  const proceedLocally = async () => {
    try {
      ...
      setDoneMessage(
        "Local mode enabled. Agents you create now will be stored on this device. To sign into Letta Cloud later, run `ppa-runtime setup` or `ppa-runtime backend cloud`.",
      );
      setMode("done");
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <LettaLoginView
          ...
          successMessage="Signed in with Letta. Starting PPA Runtime..."
        />
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <Text> </Text>
      <Text bold>Welcome to PPA Runtime</Text>
      <Text dimColor>
