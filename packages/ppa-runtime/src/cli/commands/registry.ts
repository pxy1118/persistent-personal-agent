...
export const commands: Record<string, Command> = {
  ...
  "/agents": {
    desc: "Browse agents (pinned, PPA, all)",
    order: 10,
    noArgs: true,
    handler: () => {
  ...
  "/link": {
    desc: "Attach all PPA Runtime tools to agent (deprecated, use /toolset instead)",
    hidden: true,
    noArgs: true,
    handler: () => {
      // Handled specially in App.tsx to access agent ID and client
  ...
  "/unlink": {
    desc: "Remove all PPA Runtime tools from agent (deprecated, use /toolset instead)",
    hidden: true,
    noArgs: true,
    handler: () => {
      // Handled specially in App.tsx to access agent ID and client
