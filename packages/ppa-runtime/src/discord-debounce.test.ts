...
describe("resolveDiscordInboundDebounceMs", () => {
  test("returns disabled default when config is undefined", () => {
    expect(
  ...
  test("returns disabled default for NaN config value", () => {
    expect(resolveDiscordInboundDebounceMs({ inboundDebounceMs: NaN })).toBe(0);
  });

  test("returns disabled default for negative config value", () => {
